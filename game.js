...
let deck = [];
let pot = 0;
let gameStage = 'waiting'; // waiting, preflop, flop, turn, river, showdown
let playerBet = 0;
let botBet = 0;
...
// Показываем, что приложение открыто в Telegram
...
function displayCard(element, card) {
    ...
    element.className = 'card';
    element.style.backgroundImage = `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 140"><rect width="100" height="140" fill="white" stroke="%23333" stroke-width="2"/><text x="10" y="30" font-size="24" fill="${card.color}" font-family="Arial">${card.value}</text><text x="90" y="120" font-size="24" fill="${card.color}" transform="rotate(180, 90, 120)" font-family="Arial" text-anchor="end">${card.value}</text><text x="50" y="80" font-size="40" text-anchor="middle" fill="${card.color}" font-family="Arial">${card.suit}</text></svg>')`;
}
...
function nextStage() {
    ...
    // Сбрасываем ставки для нового раунда торговли
    playerBet = 0;
    botBet = 0;
}
...
function showCards() {
    ...
    gameStatusEl.textContent = 'Вскрытие... Определяем победителя';
    
    setTimeout(() => {
        // Просто рандом для демо (в реальном покере тут сложная логика)
...
callBtn.addEventListener('click', () => {
    let callAmount = botBet - playerBet;
    if (callAmount > 0) {
        if (player.chips >= callAmount) {
            ...
            gameStatusEl.textContent = 'Недостаточно фишек!';
            return;
        }
    } else {
...
raiseBtn.addEventListener('click', () => {
    let raiseAmount = prompt('Введите сумму повышения (мин. 20):', '20');
